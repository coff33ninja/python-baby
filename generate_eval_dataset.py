...
def create_sample_tasks():
    """
    Defines a list of sample evaluation tasks.
    """
    tasks = [
        {
            "task_id": "baby_codegen_001",
        ...
        },
        {
            ...
            "stage": "toddler",
            "prompt": "Explain what this Python code does, focusing on the list comprehension.",
            "reference_code": "numbers = [1, 2, 3, 4, 5]\nsquares = [n**2 for n in numbers if n % 2 == 0]\nprint(squares)",
            "reference_text": [
        ...
        },
        {
            ...
            "task_type": "code_generation",
            "stage": "teenager",
            "prompt": "Write a Python class `Rectangle` with a constructor that accepts `width` and `height`, and methods `area()` and `perimeter()`.",
            "unit_tests": (
                "r1 = Rectangle(10, 5)\n"
                "assert r1.area() == 50\n"
        ...
        { # Task with no stage defined, for testing stage filtering in evaluate.py
            ...
            "task_type": "code_generation",
            "prompt": "Write a Python function `is_palindrome` that checks if a string is a palindrome. It should ignore case and non-alphanumeric characters.",
            "unit_tests": (
        ...
        { # Task with a different stage for filtering
            ...
            "task_type": "concept_explanation",
            "stage": "adult",
            "prompt": "Explain the Global Interpreter Lock (GIL) in CPython and its implications for multi-threaded performance.",
            "reference_text": [
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a sample evaluation dataset in .jsonl format."
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default="sample_evaluation_dataset.jsonl",
        help="Path to the output .jsonl file. Default: sample_evaluation_dataset.jsonl",
    )
    ...
    try:
        with open(args.output_file, "w", encoding="utf-8") as f:
            ...
            # Add a malformed line for testing evaluate.py's robustness
            f.write("this is not a valid json line\n")

            # Add a valid line after the malformed one
            f.write(json.dumps({
                "task_id": "another_baby_task_002",
                "task_type": "code_generation",
                "stage": "baby",
                "prompt": "Write a function `multiply` that returns the product of two numbers.",
                "unit_tests": "assert multiply(3, 4) == 12"
            }) + "\n")

        print(f"Successfully generated evaluation dataset: {args.output_file}")
        print(f"Total tasks generated (including one malformed and one extra valid): {len(tasks) + 2}")
